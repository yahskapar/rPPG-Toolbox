...
class UCLASYNTHLoader(BaseLoader):
    ...
    def get_raw_data(self, data_path):
        ...
        data_dirs = glob.glob(data_path + os.sep + "*.h5")
        if not data_dirs:
            raise ValueError(self.dataset_name + " data paths empty!")
        dirs = list()
        for data_dir in data_dirs:
            subject = os.path.splitext(os.path.split(data_dir)[-1])[0]
    ...
    def preprocess_dataset_subprocess(self, data_dirs, config_preprocess, i, file_list_dict):
        ...
        h5file_path = data_dirs[i]['path']
        saved_filename = data_dirs[i]['index']

        db = h5py.File(h5file_path, 'r')
        frames = np.asarray(db['dataset_1'])
        frames = frames.astype(np.uint8)
        bvps = np.asarray(db['ppg'])
